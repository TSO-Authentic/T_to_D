...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0utils\precheck_connections.ps1"
if %errorlevel% neq 0 (
    echo Pre-connection check failed. Pipeline aborted.
    if exist "%~dp0\utils\failure_log.txt" del "%~dp0\utils\failure_log.txt"
    goto :eof
)
